...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            Util util = new Util();
            oct_to_dectest octToDecimalTest = new oct_to_dectest();
            boolean failures = false;
            for (TestCase testCase : util.testcases) {
                String octal = testCase.getInput();
                if (octal != null) {
                    int expectedDecimal = testCase.getOutput();
                    boolean result = octToDecimalTest.test(octal, expectedDecimal);
                    testCase.setResult(result);
                    if (!result) {
                        System.out.println("Test case failed: Octal input: " + octal + ", Expected Decimal: " + expectedDecimal);
                        failures = true;
                    }
                } else {
                    System.out.println("Skipping test case with null octal input");
